...
bool CardEffect::init()
{
	...
	for (int i = 0; i < 15;i++)
	{
		...
		{
			addChild(_card_2);
			_card_2->setPosition(CommonFunction::getVisibleAchor(Anchor::MidTop, Vec2(0, -_card_2->getContentSize().width / 2)));
			_card_2->setScale(0.6f);
		...
		{
			addChild(_card_3);
			_card_3->setPosition(CommonFunction::getVisibleAchor(Anchor::MidTop, Vec2(0, -_card_3->getContentSize().width / 2)));
			_card_3->setScale(0.6f);
...
void CardEffect::midAction()
{
	if (!_cardList_1.empty())
	{
		...
		{
			_cardList_1.at(_midIndex)->runAction(spawn);
			_midIndex++;
		}
		else
		{
...
void CardEffect::rightActoin()
{
	...
	{
		auto moveTo = MoveTo::create(0.5f, CommonFunction::getVisibleAchor(Anchor::MidTop, Vec2(200, -_cardList_2.at(0)->getContentSize().width / 2)));
		auto fadeOut = FadeOut::create(0.5f);
		auto spawn = Spawn::create(moveTo, fadeOut, nullptr);

		if (_ritIndex < _cardList_2.size())
		{
			_cardList_2.at(_ritIndex)->runAction(spawn);
			_ritIndex++;
		}
		else
		{
...
void CardEffect::leftAcrion()
{
	...
	{
		auto moveTo = MoveTo::create(0.5f, CommonFunction::getVisibleAchor(Anchor::MidTop, Vec2(-200, -_cardList_3.at(0)->getContentSize().width / 2)));
		auto fadeOut = FadeOut::create(0.5f);
		auto spawn = Spawn::create(moveTo, fadeOut, nullptr);

		if (_leftIndex < _cardList_2.size())
		{
			_cardList_3.at(_leftIndex)->runAction(spawn);
			_leftIndex++;
		}
		else
		{
